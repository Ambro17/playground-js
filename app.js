...
const defaults = {
  playground_port: 4040,
  endpoint: 'https://graphql-pokemon.now.sh/',  // replace by any other public api for quick start
}

const playground_port = process.env.PLAYGROUND_PORT || defaults.playground_port
const api_url = process.env.GRAPHQL_API_ENDPOINT || defaults.endpoint
...
app.get('/', (req, res) => res.send(enriched_html))

app.listen(playground_port, function(error) {
  if (error) console.error(error)
  console.log(
    `GraphQL api endpoint: ${api_url}`,
    `\n\nGraphQL Playground on http://localhost:${playground_port}`
  )
})
